...
if %ifAlt% == 1 (
   call gams aggGTAP --basename=%baseName% --ifAlt=ON --model=%Model% --ifCSV=1 --ifAggTrade=1 -pw=132 -ps=9999 -errmsg=1
   echo.ErrorLevel = !errorlevel!
) else (
   call gams aggGTAP --basename=%baseName% --ifAlt=OFF --model=%Model% --ifCSV=1 --ifAggTrade=1 -pw=132 -ps=9999 -errmsg=1
   echo.ErrorLevel = !errorlevel!
)
if %errorlevel% NEQ 0 (
   goto BadAggregation
...
if !Model! == Env (
   call gams convertLabel --baseName=%baseName% -pw=150 -ps=9999 -errmsg=1
   echo.ErrorLevel = !errorlevel!
   if !errorlevel! NEQ 0 (
      goto BadConversion
   ) else (
...
if %ifFilter% == 0 (
   ...
   call gams filter --BaseName=%baseName% --ifCSV=1 --ifAggTrade=1 --Macro=OFF -pw=132 -ps=9999 -errmsg=1
   echo.ErrorLevel = !errorlevel!
   if !errorlevel! NEQ 0 (
      goto BadFilter
   )
...
if %ifAlt% == 0 (
   ...
   :: Run Altertax
   call gams AlterTax --BaseName=%baseName% --niter=1 --ifMRIO=0 --ifCSV=1 -idir=GTAPModel -pw=150 -errmsg=1
   echo.ErrorLevel = !errorlevel!
   if !errorlevel! NEQ 0 (
      goto BadAlterTax
   )
