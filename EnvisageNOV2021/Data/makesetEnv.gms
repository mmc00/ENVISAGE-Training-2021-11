...
loop(actf$(not (acr(actf) or alv(actf))),
   ...
   put '   ', actf.tl:<strlen, '-a', ' ':(maxstrlen-(strlen+2)+5), '"', actf.te(actf), '"' / ;
$iftheni "%ifBKSTOP%" == "ON"
*     Is this where we insert the backstops?
      if(aloc(actf),
         loop(bkstp,
            strlen = card(bkstp.tl) ;
            put '   ', bkstp.tl:<strlen, '-a', ' ':(maxstrlen-(strlen+2)+5), '"', bkstp.te(bkstp), '"' / ;
         ) ;
      ) ;
$endif
) ;
...
put 'set CPINDX "Categories of CPI indices" /' / ;
loop(cpindx,
   strlen = card(cpindx.tl) ;
   put '   ', cpindx.tl:<strlen, ' ':(maxstrlen-(strlen+2)+5), '"', cpindx.te(cpindx), '"' / ;
) ;
put '/ ;' / / ;

put 'set mapCPI(cpindx,i) "Mapping from i to CPINDX" /' / ;
loop(mapCPI(cpindx,commf),
   strlen = card(cpindx.tl) ;
   put '   ', cpindx.tl:<strlen, ' ':(maxstrlen-(strlen+2)+1), '.', commf.tl:card(commf.tl), '-c' / ;
) ;
put '/ ;' / / ;
...
$iftheni "%ifBKSTOP%" == "ON"
   loop(mappowbkstp(pb,bkstpely),
   strlen = card(pb.tl) ;
   put '   ', pb.tl:<strlen, ' ':(maxstrlen-(strlen)+1), '.', bkstpely.tl:card(bkstpely.tl), '-a' / ;
   ) ;
$endif
put '/ ;' / / ;
...
put 'set pt "Price trends" / lo, mid, ref, hi / ;' / / ;
...
put 'set emq "Emission control aggregations" /' / ;
put '   CO2         "Carbon dioxide"' / ;
put '   CO2e        "Carbon dioxide equivalent"' / ;
put '/ ;' / / ;


put 'set mapEM(emq,em) /' / ;
put 'CO2.CO2' / ;
put '/ ;' / / ;

put 'mapEM("CO2e",em)$ghg(em) = yes ;' / / ;
...
put 'set GHGCodes "GHG codes from WB WDI database" /' / ;
loop(GHGCodes,
   strlen = card(GHGCodes.tl) ;
   put '   ', GHGCodes.tl:<strlen, ' ':(maxstrlen-(strlen)+5), '"', GHGCodes.te(GHGCodes), '"' / ;
) ;
put '/ ;' / / ;
...
put 'set aets "Agent specific ETS coalitions" /' / ;
loop(aets,
   strlen = card(aets.tl) ;
   put '   ', aets.tl:<strlen, ' ':(maxstrlen-(strlen+2)+5), '"', aets.te(aets), '"' / ;
) ;
* put '   ', '"All"':<5, ' ':(maxstrlen-(5+2)+5), '"', 'All agents', '"' / ;
put '/ ;' / / ;

put 'set mapETS(aets,aa) "Mapping of agents to ETS coalitions" /' / ;
loop(mapETS(aets,aa),
   strlen = card(aets.tl) ;
   if(sum(actf$sameas(actf,aa), 1),
      put '   ', aets.tl:<strlen, ' ':(maxstrlen-(strlen+2)+1), '.', aa.tl:card(aa.tl), '-a' / ;
   else
      put '   ', aets.tl:<strlen, ' ':(maxstrlen-(strlen+2)+1), '.', aa.tl:card(aa.tl) / ;
   ) ;
) ;
put '/ ;' /
put 'mapETS("All", aa) = yes ;' / / ;
