...
*
   ...
   loop(i,
      total("old",r) = sum(rp, exi0(i,r,rp));
      total("new",r) = sum(rp $ vfob(i,r,rp), exi(i,r,rp));
      exi(i,r,rp) $ total("new",r) = (exi(i,r,rp) * total("old",r)/total("new",r)) $ vfob(i,r,rp) ;
   ) ;
...
*     Save original emissions data for scaling purposes
      ...
      Parameters
         EMI_HH0(em,i,r), EMI_IO0(em,i,a,r), EMI_IOP0(em,i,a,r)
            EMI_ENDW0(em,fp,a,r), EMI_QO0(em,j,r) ;

      EMI_HH0(em,i,r)           = EMI_HH(em,i,r) ;
      EMI_IO0(em,i,a,r)         = EMI_IO(em,i,a,r) ;
      EMI_IOP0(em,i,a,r)        = EMI_IOP(em,i,a,r) ;
      EMI_ENDW0(em,fp,j,r)      = EMI_ENDW(em,fp,j,r) ;
      EMI_QO0(em,j,r)           = EMI_QO(em,j,r) ;
...
*        Scale individual cells first
         ...
         EMI_IO(em,i,j,r)$(vdfb0(i,j,r) + vmfb0(i,j,r))
            = (EMI_IO0(em,i,j,r)/(vdfb0(i,j,r) + vmfb0(i,j,r)))*(vdfb(i,j,r) + vmfb(i,j,r)) ;

         EMI_IOP(em,i,j,r)$(vdfb0(i,j,r) + vmfb0(i,j,r))
            = (EMI_IOP0(em,i,j,r)/(vdfb0(i,j,r) + vmfb0(i,j,r)))*(vdfb(i,j,r) + vmfb(i,j,r)) ;

         EMI_ENDW(em,fp,j,r)$(evfb0(fp,j,r))
            = (EMI_ENDW0(em,fp,j,r)/evfb0(fp,j,r))*evfb(fp,j,r) ;

         EMI_QO(em,j,r)$(vom0(j,r))
            = (EMI_QO0(em,j,r)/vom0(j,r))*vom(j,r) ;
      ) ;
...
*        Output
         ...
         total("old",r) = sum(j, EMI_QO0(em,j,r)) ;
         total("new",r) = sum(j $ vom(j,r), EMI_QO(em,j,r)) ;
         EMI_QO(em,j,r) $ total("new",r) = (EMI_QO(em,j,r) * total("old",r)/total("new",r)) $ vom(j,r) ;
...
*  Use a 'RAS' type procedure to ensure consistency
   ...
   put logfile ; put / "MRIO iterations: " / / ;
