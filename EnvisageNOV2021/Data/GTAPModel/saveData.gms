...
*  Energy volumes
   ...
   exi(i0, s, r)     "Bilateral trade in energy"
...
*  Energy volumes
   ...
   exi0(i0, s, r)       "Bilateral trade in energy"

   EMI_IO0(EM, i0, a0, r)     "IO-based emissions"
   EMI_IOP0(EM, i0, a0, r)    "IO-based process emissions"
   EMI_ENDW0(EM, fp, a0, r)   "Endowment-based emissions"
   EMI_QO0(EM, a0, r)         "Output-based emissions"
   EMI_HH0(EM, i0, r)         "Private consumption-based emissions"
...
*     Scale individual cells first
      ...
      exi(i0,r,d)$vfob(i0,r,d)   = (exi0(i0,r,d)/vfob(i0,r,d))*VFOBF(i0,r,d) ;
   ) ;
...
*
   if(0,
      ...
      loop(i0,
         total("old",r) = sum(rp, exi0(i0,r,rp));
         total("new",r) = sum(rp $ VFOBF(i0,r,rp), exi(i0,r,rp));
         exi(i0,r,rp) $ total("new",r) = (exi(i0,r,rp) * total("old",r)/total("new",r)) $ VFOBF(i0,r,rp) ;
      ) ;
   ) ;
...
$ifthen exist "%baseName%/flt/%baseName%nco2.gdx"
   ...
   sets
      AR       "IPPC Reports"
      GHG(em)  "Greenhouse gases"
      NGHG(em) "Non-greehnouse gases"
   ...
   $$gdxin "%baseName%/flt/%baseName%nco2.gdx"
   $$load GHG, NGHG, LU, LUType

   execute_loaddc  "%baseName%/flt/%baseName%nco2.gdx",
     GWP, EMI_IO, EMI_IOP, EMI_ENDW, EMI_QO, EMI_HH, EMI_LU ;

*  Save initial levels for re-scaling

   EMI_HH0(em,i0,r)           = EMI_HH(em,i0,r) ;
   EMI_IO0(em,i0,a0,r)        = EMI_IO(em,i0,a0,r) ;
   EMI_IOP0(em,i0,a0,r)       = EMI_IOP(em,i0,a0,r) ;
   EMI_ENDW0(em,fp,a0,r)      = EMI_ENDW(em,fp,a0,r) ;
   EMI_QO0(em,a0,r)           = EMI_QO(em,a0,r) ;
...
*     Scale individual cells first
      ...
      EMI_HH(em,i0,r)$(vdpb(i0,r) + vmpb(i0,r))
         = (EMI_HH0(em,i0,r)/(vdpb(i0,r) + vmpb(i0,r)))*(VDPBF(i0,r) + VMPBF(i0,r)) ;

      EMI_IO(em,i0,j0,r)$(vdfb(i0,j0,r) + vmfb(i0,j0,r))
         = (EMI_IO0(em,i0,j0,r)/(vdfb(i0,j0,r) + vmfb(i0,j0,r)))*(VDFBF(i0,j0,r) + VMFBF(i0,j0,r)) ;
      EMI_IOP(em,i0,j0,r)$(vdfb(i0,j0,r) + vmfb(i0,j0,r))
         = (EMI_IOP0(em,i0,j0,r)/(vdfb(i0,j0,r) + vmfb(i0,j0,r)))*(VDFBF(i0,j0,r) + VMFBF(i0,j0,r)) ;

      EMI_ENDW(em,fp,j0,r)$(evfb(fp,j0,r))
         = (EMI_ENDW0(em,fp,j0,r)/evfb(fp,j0,r))*EVFBF(fp,j0,r) ;

      EMI_QO(em,j0,r)$(tvom(j0,r))
         = (EMI_QO0(em,j0,r)/tvom(j0,r))*tvomf(j0,r) ;
   ) ;
...
*        Households
         ...
         total("old",r) = sum(i0, EMI_HH0(em,i0,r)) ;
         total("new",r) = sum(i0 $ (VDPBF(i0,r) + VMPBF(i0,r)), EMI_HH(em,i0,r)) ;
         EMI_HH(em,i0,r) $ total("new",r)
            = (EMI_HH(em,i0,r) * total("old",r)/total("new",r)) $ (VDPBF(i0,r) + VMPBF(i0,r)) ;

*        Firms

         total("old",r) = sum((i0,a0), EMI_IO0(em,i0,a0,r));
         total("new",r) = sum((i0,a0) $ (VDFBF(i0,a0,r) + VMFBF(i0,a0,r)), EMI_IO(em,i0,a0,r));
         EMI_IO(em,i0,a0,r)$ total("new",r)
            = (EMI_IO(em,i0,a0,r) * total("old",r)/total("new",r)) $ (VDFBF(i0,a0,r) + VMFBF(i0,a0,r)) ;

         total("old",r) = sum((i0,a0), EMI_IOP0(em,i0,a0,r));
         total("new",r) = sum((i0,a0) $ (VDFBF(i0,a0,r) + VMFBF(i0,a0,r)), EMI_IOP(em,i0,a0,r));
         EMI_IOP(em,i0,a0,r)$ total("new",r)
            = (EMI_IOP(em,i0,a0,r) * total("old",r)/total("new",r)) $ (VDFBF(i0,a0,r) + VMFBF(i0,a0,r)) ;

*        Endowments

         total("old",r) = sum((fp,a0), EMI_ENDW0(em,fp,a0,r));
         total("new",r) = sum((fp,a0) $ (EVFBF(fp,a0,r)), EMI_ENDW(em,fp,a0,r));
         EMI_ENDW(em,fp,a0,r)$ total("new",r)
            = (EMI_ENDW(em,fp,a0,r) * total("old",r)/total("new",r)) $ (EVFBF(fp,a0,r)) ;

*        Output

         total("old",r) = sum(j0, EMI_QO0(em,j0,r)) ;
         total("new",r) = sum(j0 $ tvomf(j0,r), EMI_QO(em,j0,r)) ;
         EMI_QO(em,j0,r) $ total("new",r)
            = (EMI_QO(em,j0,r) * total("old",r)/total("new",r)) $ tvomf(j0,r) ;

      ) ;
   ) ;

   execute_unload "%baseName%/Alt/%baseName%NCO2.gdx",
      ar, i0=comm, fp=endw, a0=acts, r=reg, em, emn, ghg, nghg, lu, luType,
      GWP, emi_IO, emi_IOP, emi_ENDW, emi_QO, emi_HH, emi_LU ;
