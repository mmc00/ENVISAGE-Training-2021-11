...
maxstrlen = max(maxstrlen, smax(wb, card(wb.tl))+2) ;
...
*     Is this where we insert the backstops?
      if(aloc(actf),
         loop(bkstp,
            ...
            put '   ', bkstp.tl:<strlen, '-a', '':(maxstrlen-strlen-2), '     "', bkstp.te(bkstp), '"' / ;
         ) ;
      ) ;
$endif
   ) ;
...
*!!!! For compatibility -- assume first labor bundle has the unskilled workers

put '*   Subsets ul(l) and sl(l) included for backward compatibility' / / ;
put 'set ul(l) "Unskilled labor" /' / / ;
loop(mapl(wb,l)$(ord(wb) eq 1),
   put '   ', l.tl:<maxstrlen, '     "', l.te(l), '"' / ;
) ;
put '/ ;' / / ;

put 'set sl(l) "Skilled labor" /' / / ;
loop(mapl(wb,l)$(ord(wb) ne 1),
   put '   ', l.tl:<maxstrlen, '     "', l.te(l), '"' / ;
) ;
put '/ ;' / / ;

put 'set wb "Intermediate labor bundle(s)" /' / ;
loop(wb,
   put '   ', wb.tl:<maxstrlen, '     "', wb.te(wb), '"' / ;
) ;
put '/ ;' / / ;

put 'set maplab1(wb) "Mapping of intermediate labor demand bundle(s) to LAB1" /' / ;
loop(wb$maplab1(wb),
   put '   ', wb.tl:<maxstrlen, '     "', wb.te(wb), '"' / ;
) ;
put '/ ;' / / ;

put 'set mapl(wb,l) "Mapping of labor categories to intermedate demand bundle(s)" /' / ;
loop(mapl(wb,l),
   strlen = card(wb.tl) ;
   put '   ', wb.tl:<strlen, ' ':(maxstrlen-(strlen)+1), '.', l.tl:card(l.tl) / ;
) ;
put '/ ;' / / ;
...
put 'singleton set gov(fd) "Government" /' / ;
loop(fd$gov(fd),
   put '   ', fd.tl:<maxstrlen, '     "', fd.te(fd), '"' / ;
) ;
put '/ ;' / / ;
...
$iftheni "%ifR_D%" == "ON"
put 'singleton set r_d(fd) "Research and development" /' / ;
loop(fd$r_d(fd),
   put '   ', fd.tl:<maxstrlen, '     "', fd.te(fd), '"' / ;
) ;
put '/ ;' / / ;
...
$iftheni "%ifIFI%" == "ON"
put 'singleton set ifi(fd) "International financial institutions" /' / ;
loop(fd$ifi(fd),
   put '   ', fd.tl:<maxstrlen, '     "', fd.te(fd), '"' / ;
) ;
put '/ ;' / / ;
...
put 'singleton set inv(fd) "Investment" /' / ;
loop(fd$inv(fd),
   put '   ', fd.tl:<maxstrlen, '     "', fd.te(fd), '"' / ;
) ;
put '/ ;' / / ;
...
*     Is this where we insert the backstops?
      if(aloc(actf),
         loop(bkstp,
            strlen = card(bkstp.tl) ;
            put '   ', bkstp.tl:<strlen, '-a', ' ':(maxstrlen-(strlen+2)+1), '.', bkstp.tl:card(bkstp.tl), '-a' / ;
         ) ;
      ) ;
$endif
   ) ;
...
$iftheni "%ifBKSTOP%" == "ON"
   loop(mapabkstp(aga,bkstp),
      strlen = card(aga.tl) ;
      put '   ', aga.tl:<strlen, ' ':(maxstrlen-(strlen)+1), '.', bkstp.tl:card(bkstp.tl), '-a' / ;
   ) ;
$endif
   put '/ ;' / / ;
else
...
put 'set maplagg(lagg,l) "Mapping for aggregate regions" /' / ;
loop(l,
   strlen = card(l.tl) ;
...
if(%1 eq AlterTax,
   ...
   loop(mapaf(a,actf),
      strlen = card(a.tl) ;
      put '   ', a.tl:<maxstrlen, '.', actf.tl:<card(actf.tl), '-a' / ;
   ) ;
) ;
put '/ ;' / / ;
...
*     Is this where we insert the backstops?
      if(aloc(actf),
         loop(bkstp,
            order = order + 1 ;
            put 'sort', order:0:0, '.', bkstp.tl:card(bkstp.tl), '-a' / ;
         ) ;
      ) ;
$endif
   ) ;
